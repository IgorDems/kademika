...
<div id="navigate">
<dl><%int index = 0;ArrayList <String[]> buttons = MenuFactory.proccessRequest(request); if(buttons.size() > 0){%><dt><div class="menu_section">Направления</div></dt><%} for(String [] button : buttons){%>
<%if(index == 0){index = 1; %><dd><div class="menu_row1" onclick="javascript:changeSelect(<%="'"+ button[0]+"'"%>);"><%=button[1]%></div></dd><%}
else{index = 0;%><dd><div class="menu_row2" onclick="javascript:changeSelect(<%="'"+ button[0]+"'"%>);"><%=button[1]%></div></dd><%}%>
<%}%></dl></div>
<div id="content">
...
<select id="grouping">
	<option selected value="0">Без группировки</option>
...
<%if (request.getParameter("from") != null && request.getParameter("to") != null) { for(String names : ChartFactory.proccessRequest(request)){%><div class="stat">
<img src="res/loading.gif" onload="javascript:getImage(this, '<%=Constants.getUrl(request)%>statistic<%=Constants.getDirectionParams(request, names.substring(0, names.indexOf("/")), names.substring(names.indexOf("/")+1))%>');" width="235" height="235" border=0 alt="">
	</div>
<%}}%> <%}else {%>Выберите сервер в боковом меню<%}%>
</div>
</div>
</body>